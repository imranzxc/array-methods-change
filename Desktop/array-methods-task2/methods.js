...
const arrayFind = array.find((element) => element == array[id - 1])
return arrayFind
}
...
function deleteUser(array, id) {
  const deleteuser = array.filter((element) => element.id !== id)
  return deleteuser
  }
...
const geo = array.find((element) => element.id === id)
return geo.address.geo
}
...
function getUsersCompany(array, id) {
  const companygetter = array.find((element) => element.id === id)
  return companygetter.company
}
